...
def remove(string):
    with open(
        f"C:/PythonProgrammingProject_main-folder/Student.csv", "r+", newline=""
    ) as f:
        script = csv.reader(f)
...
while message:
    ...
    with open(f"{path}/Department.csv", "a", newline="") as f:
        ...
        script.writerow(
            [
                "CSEAIML",
                "Computer Science and Engineering and Artificial Intelligence and Machine Learning",
            ]
        )
        script.writerow(
            [
                "CSEIOTCSBS",
                "Computer Science and Engineering and Internet of Things and Business Studies",
            ]
        )
...
for i in range(student_no):
    ...
    batch = input("which batch he/she is in (e.g. 2022-26) : ")
    ...
    if duplicate("Student.csv", student_id, 0):
        ...
        print(
            f"You can find your report card here : {path}/ReportCards/{student_id}_{name}.txt"
        )
    else:
        print()
        ...
        with open(
            f"{path}/ReportCards/{student_id}_{''.join(name.split())}.txt", "w"
        ) as f:
            ...
            f.write("\n")
            f.write(
                f"You have got {total_marks} in total with {percent(total_marks)}%\n"
            )
            f.write(grade(total_marks / count(marks_lst)))
        createfile("Student.csv", [student_id, name, roll, batch_id])
        print(
            f"You can find your report card here : {path}/ReportCards/{student_id}_{''.join(name.split())}.txt"
        )
        openpath = f"{path}/ReportCards/{student_id}_{''.join(name.split())}.txt"
        subprocess.run(["start", openpath], shell=True)

        ask = input(
            "Do you want to remove this name from database now is the time (Y/N) : "
        )
        ...
        if ask.lower() == "n":
            if duplicate("Batch.csv", batch_id, 0):
                ...
                createfile(
                    "Batch.csv",
                    [batch_id, batch_name, stream, choice(stream), student_id],
                )
